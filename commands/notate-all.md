...
- **Back up every file that will lose content first** - `~/.claude/CLAUDE.md` before any inline rule edit, and equally any note or memory file an `UPDATE` will delete a line from - exactly as Step 6 item 3 specifies. The snapshot must precede the first write, or it preserves the damage rather than the original.
