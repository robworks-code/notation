...
4. Offer to apply fixes. Apply only what the user approves. Before editing `~/.claude/CLAUDE.md`, back it up: `cp ~/.claude/CLAUDE.md ~/.claude/CLAUDE.md.bak.$(date +%Y%m%d-%H%M%S)`. While the source text is still in front of you, pick 1-2 distinctive probe strings per move (`references/verify-after-apply.md`).
5. **Verify after applying, in this order:** first the **preservation probe** - confirm each moved fact actually landed in its destination file - then re-measure (`wc -c ~/.claude/CLAUDE.md`) and report before -> after -> target with real numbers. If a probe fails, restore from the backup and report the loss; do not report the size win. If the run ended net-positive, or still over target, say so and offer the next round. Full procedure: `references/verify-after-apply.md`.
...
**Size is the half of the verification that lies.** A relocation is two writes - remove from CLAUDE.md, append to the note - so if the append fails while the removal succeeds, the file gets *smaller* and every size check passes. Shrinking is the failure signature of data loss. Never finish a run on `wc -c` alone: prove each moved fact reached its destination first, and restore from the backup if one did not. Procedure: `references/verify-after-apply.md`.

Full detail and the routing rubric used to decide where a stray entry should move: `references/audit-checklist.md`, `references/routing-rubric.md`, `references/size-budget.md`, and `references/verify-after-apply.md`.
