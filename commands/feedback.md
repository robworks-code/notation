---
description: Send feedback about the notation plugin (a bug, an idea, or a routing correction) straight to the maintainer's support queue as a ticket. Attaches lightweight context - plugin version, OS, and a short summary of what notation did this session. Confirms before sending.
...
Collect only these, nothing more (no transcript, no file paths, no secrets):
...
- **session_summary** - one short line on what notation did this session if relevant
  (e.g. "routed 3 learnings; 1 railway note, 2 memory files"), else omit. Keep it to a
  sentence; never paste conversation content.
