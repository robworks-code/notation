...
**Every relocation is proved, not assumed.** A move is two writes - remove from `CLAUDE.md`, append to the note - so if the append fails while the removal succeeds, the file gets *smaller* and a size check passes. Shrinking is the failure signature of data loss. Before applying, the audit picks one or two distinctive strings from each block being moved (an error message, a sha, a flag - never a common word); afterward it confirms each one landed in its destination file and reports the tally (`Preservation: 22/22 relocated phrases verified`). If any probe misses, it restores from the backup and reports the loss rather than the size win.

Character deltas are **measured, not felt**: each row's number is the byte count of the content being removed minus the replacement line, which is drafted at report time so both halves are real. Rows whose replacement length was assumed rather than drafted are marked with a `~`. The projected total is still labelled an estimate, because estimates here skew optimistic - the audit expects to converge over two or three passes and says so instead of promising a landing it cannot make in one.
