...
4. Offer to apply fixes. Apply only what the user approves. **Before the first write, back up every file the run will remove content from** - `~/.claude/CLAUDE.md`, plus any note being consolidated away and `./CLAUDE.md` on a strict run - using one timestamp for the whole set. While the source text is still in front of you, pick 1-2 distinctive probe strings per relocation and assert their preconditions (`references/verify-after-apply.md`).
