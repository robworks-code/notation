---
description: Send feedback about the notation plugin (a bug, an idea, or a routing correction) straight to the maintainer's support queue as a ticket. Offers to attach your email and extra context - and shows exactly what it collects. Confirms before sending.
...
routing decision that went wrong.
...
> else. No file paths, no learnings content, no memory or note contents, no conversation.
...
- **Add more context** - one sentence on what you were doing (e.g. "routed 3 learnings; 1
  railway note, 2 memory files"). If chosen, capture a single line as `session_summary`.
  Never paste learnings, note, or conversation content.
...
`<scratchpad>/notation-feedback.json` (use the session scratchpad dir). Building it with
...
curl -sS -X POST "https://support.robworks.info/api/feedback" \
  ...
  --data @"<scratchpad>/notation-feedback.json" \
  -w '\nHTTP %{http_code}\n'
```
